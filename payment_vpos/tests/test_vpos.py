...
class vPOSPayment(PaymentAcquirerCommon):
    ...
    def test_10_vpos_form_render(self):
        ...
        self.assertEqual(tree.get('action'), 'https://secure.vpos.com/ncol/test/orderstandard.asp', 'vpos: wrong form POST url')
        for form_input in tree.input:
            ...
            self.assertEqual(
                ...
                'vpos: wrong value for input %s: received %s instead of %s' % (form_input.get('name'), form_input.get('value'), form_values[form_input.get('name')])
            )
